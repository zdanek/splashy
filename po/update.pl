...
if ($LANG =~ /^-(.)*/)
{
    ...
    elsif ($LANG eq "--help" || "$LANG" eq "-H")
    {
        ...
        print "\nExamples of use:\n";
        print
          "update.pl --pot    just creates a new pot file from the source\n";
        print
          "update.pl da       created new pot file and updated the da.po file\n\n";
    ...
    elsif ($LANG eq "--maintain" || "$LANG" eq "-M")
    {
        ...
        $a =
          "find ../ -path ../intl -prune -o -print | egrep '.*\\.(c|y|cc|c++|h|gob)\$' ";

        open(BUF2, "POTFILES.in")
          || die "update.pl:  there's not POTFILES.in!!!\n";
        print
          "Searching for missing _(\" \") entries and for deleted files...\n";
        open(BUF1, "$a|");
        ...
        if (@result)
        {
            ...
            print
              "Please add the files that should be ignored in POTFILES.ignore\n";
        }
        else
        {
        ...
        if (@deletedfiles)
        {
            ...
            print
              "Please delete the files from POTFILES.in or POTFILES.ignore\n";
        }
        else
        {
...
elsif (-s "$LANG.po")
{
    ...
    if ($ret == 0)
    {
        ...
        $d =
          ...
          . "  msgfmt --statistics  -c -v -o /dev/null $LANG.po; " . "else "
